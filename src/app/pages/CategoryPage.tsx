...
import { getArticlesByCategory, categories, type Category } from '../data/mockData';
import { motion } from 'motion/react';

export const CategoryPage = () => {
  const { category } = useParams<{ category: string }>();
  category; // Suppress unused var check if needed or just use it.
  const categoryInfo = categories.find(cat => cat.slug === category);
  const articles = category ? getArticlesByCategory(category as Category) : [];
    ...
    <div className="min-h-screen text-gray-900 dark:text-gray-100 pb-16">
      ...
      <div className="relative py-20 overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-bold mb-6 text-white text-shadow-strong">{categoryInfo.name}</h1>
            <p className="text-xl md:text-2xl text-white/90 max-w-2xl text-shadow">{categoryInfo.description}</p>
            <p className="mt-4 text-white/80 font-medium">{articles.length} articles</p>
          </motion.div>
        </div>
      </div>
